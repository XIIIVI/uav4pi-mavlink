...
def generate_flatbuffer_enum_definition(filename_arg, fbsfile_arg, enum_element_arg):
    enum_definition = ''
    line_count = 0
    ...
    if (len(entry_list) > 0):
        print(f"\t\t- Generating the enum {enum_name}")
        
        for field_element in entry_list:
            ...
            enum_definition = enum_definition + \
                (f"   {field_element.get('name')}={field_element.get('value', '')}")
...
def extract_enum_info(filename_arg, fbsfile_arg, enum_element_arg):
    ...
    generate_flatbuffer_enum_definition(filename_arg, fbsfile_arg, enum_element_arg)
    fbsfile_arg.write("\n")
...
def extract_message_info(file_arg, message_element_arg, default_enum_values_map_arg):
    ...
    for field_element in message_element_arg.findall('field'):
        ...
        if (not field_element.get('enum', '').strip()):
            ...
            print(f"\t\t- {enum_name}")
            file_arg.write(f"   {field_element.get('name')}:{enum_name} = {default_enum_values_map_arg[enum_name]};\n")
        ...
        if filename.endswith(".xml"):
            ...
            for include in include_map:
                if (default_enum_values.get(include) != None):
                   all_default_values = {
                       **all_default_values, **default_enum_values[include]}
            
            default_value_map[filename] = all_default_values
    ...
    for filename in os.listdir(input_dir_arg):
        if filename.endswith(".xml"):
            ...
            with open(flatbuffer_filename, 'a') as file:
                print(
                    f"\t- Converting {filename}.xml => {include_map_arg[filename]}")
                xml_to_flatbuffers(root, file, default_value_map[filename])


def flatten_include_list(include_map_arg, include_list_arg):
    result = []

    for value in include_list_arg:
        if ( len(include_map_arg[value]) > 0 ):
            result = flatten_include_list(
                include_map_arg, include_map_arg[value])
        else:
            result.append(value)
...
def flatten_include_map(include_map_arg):
    result = []

    for key, value in include_map_arg.items():
        flattened_list = flatten_include_list(include_map_arg, value)
        
        include_map_arg[key] = list(dict.fromkeys(flattened_list))
...
def main():
    ...
    prepare_files(flattened_include_map, args.input, args.output)
    flatten_include_map(flattened_include_map)
    print(f"{flattened_include_map}")
    convert_xml_files(flattened_include_map, args.input, args.output)
